    ...
    fun `FindClaimsFlow should find the expected claim by linear ID`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(linearId = GREETING_CLAIM.linearId) }
    ...
    fun `FindClaimsFlow should find the expected claim by external ID`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(externalId = GREETING_CLAIM.linearId.externalId) }
                    .finally { assertEquals(2, it.size) }
            }
        }
    }
    ...
    fun `FindClaimsFlow should find the expected claim by issuer`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(issuer = GREETING_CLAIM.issuer) }
                    .finally { assertEquals(2, it.size) }
            }
        }
    }
    ...
    fun `FindClaimsFlow should find the expected claim by holder`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(holder = GREETING_CLAIM.holder) }
                    .finally { assertEquals(2, it.size) }
            }
        }
    }
    ...
    fun `FindClaimsFlow should find the expected claim by property`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(property = GREETING_CLAIM.property) }
                    .finally { assertEquals(2, it.size) }
            }
        }
    }
    ...
    fun `FindClaimsFlow should find the expected claim by value`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(value = GREETING_CLAIM.value) }
    ...
    fun `FindClaimsFlow should find the expected claim by isSelfIssued`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(isSelfIssued = true) }
    ...
    fun `FindClaimsFlow should find the expected claim by hash`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                Pipeline
                    .create(network)
                    .run(it) { FindClaimsFlow<GreetingClaim>(hash = GREETING_CLAIM.hash) }
                    .finally { assertEquals(1, it.size) }
            }
        }
    }
