...
class AmendAccountFlowTests : FlowTest() {
    ...
    override fun initialize() {
        Pipeline
            ...
            .run(nodeA) {
                ...
                account = oldAccount.state.data
                AmendAccountFlow.Initiator(oldAccount, account, observers = setOf(partyB, partyC))
            }
            .finally { transaction = it }
    }

    @Test
    fun `AmendAccountFlow transaction should be signed by the initiator`() {
        transaction.verifyRequiredSignatures()
    }

    @Test
    fun `AmendAccountFlow should record a transaction for the account holder and observers`() {
    ...
    @Test
    fun `AmendAccountFlow should record an Account for the account holder and observers`() {
        listOf(nodeA, nodeB, nodeC).forEach {
            it.transaction {
                val recordedTransaction = it.services.validatedTransactions.getTransaction(transaction.id)
                    ?: fail("Failed to find a recorded transaction with id: ${transaction.id}.")
