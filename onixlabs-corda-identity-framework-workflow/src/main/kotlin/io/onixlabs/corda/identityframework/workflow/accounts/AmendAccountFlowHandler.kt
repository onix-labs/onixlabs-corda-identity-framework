...
/**
 * Represents the flow handler for amending accounts.
 *
...
class AmendAccountFlowHandler(
    ...
    /**
     * Represents the initiated flow handler for amending accounts.
     *
     * @property session The counter-party session who is initiating the flow.
     */
    @InitiatedBy(AmendAccountFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {
        ...
        override val progressTracker = ProgressTracker(HandleAmendedAccountStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleAmendedAccountStep)
            return subFlow(
                AmendAccountFlowHandler(
                    session,
                    progressTracker = HandleAmendedAccountStep.childProgressTracker()
                )
            )
        }
    }
