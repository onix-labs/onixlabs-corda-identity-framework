...
/**
 * Represents the flow handler for issuing accounts.
 *
...
class IssueAccountFlowHandler(
    ...
    /**
     * Represents the initiated flow handler for issuing accounts.
     *
     * @property session The counter-party session who is initiating the flow.
     */
    @InitiatedBy(IssueAccountFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {
        ...
        override val progressTracker = ProgressTracker(HandleIssuedAccountStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleIssuedAccountStep)
            return subFlow(
                IssueAccountFlowHandler(
                    session,
                    progressTracker = HandleIssuedAccountStep.childProgressTracker()
                )
            )
        }
    }
