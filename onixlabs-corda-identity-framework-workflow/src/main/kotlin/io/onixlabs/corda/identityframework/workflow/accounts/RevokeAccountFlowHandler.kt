...
/**
 * Represents the flow handler for revoking accounts.
 *
 ...
 */
class RevokeAccountFlowHandler(
    ...
    /**
     * Represents the initiated flow handler for revoking accounts.
     *
     * @property session The counter-party session who is initiating the flow.
     */
    @InitiatedBy(RevokeAccountFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {
        ...
        override val progressTracker = ProgressTracker(HandleRevokedAccountStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleRevokedAccountStep)
            return subFlow(
                RevokeAccountFlowHandler(
                    session,
                    progressTracker = HandleRevokedAccountStep.childProgressTracker()
                )
            )
        }
    }
